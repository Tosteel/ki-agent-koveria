...
def _module_name_for_file(base_dir: Path, file_path: Path) -> str:
    ...
    if not rel_dir.parts:
        raise ValueError(f"Invalid assistant module directory: {file_path.parent}")
    return "server.assistants." + ".".join(rel_dir.parts) + f".{file_path.stem}"


def _iter_assistant_module_files(filename: str) -> list[Path]:
    project_server_dir = Path(__file__).resolve().parents[1]
    base_dir = project_server_dir / "assistants"
    files: list[Path] = []
...
def create_all_assistant_api_router(*, ensure_user_dirs: Callable) -> APIRouter:
    router = APIRouter()
    project_server_dir = Path(__file__).resolve().parents[1]
    base_dir = project_server_dir / "assistants"
    for file_path in _iter_assistant_module_files("api.py"):
