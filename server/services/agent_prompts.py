    ...
    "default": {
        "ionos": {
            "planner_system": (
                ...
                "file_read ist verboten, wenn der Nutzer keine Datei benennt. In diesem Fall MUSS rag_knowledgebase genutzt werden.\n"
                ...
                "Wenn der Nutzer nach Fähigkeiten/Tools des Agenten fragt, nutze skills_list.\n"
                "Nutze mail_send, wenn eine E-Mail an eine Adresse versendet werden soll und keine konkrete Inbox-Mail referenziert ist.\n"
                "Nutze mail_answer nur für Antworten auf eine bereits vorhandene Inbox-Mail (mit mail_id/uid oder expliziter Referenz wie 'antworte auf diese Mail').\n"
                "Für mail_answer nutze als mail_id den Platzhalter {steps[0].mail_id} oder {steps[0].emails[0].uid}.\n"
                ...
                "Wenn der Nutzer eine PDF-Datei analysieren/lesen will, nutze read_pdf (nicht file_read).\n"
        ...
        "openai": {
            "planner_system": (
                ...
                "Use skills_list when the user asks about agent capabilities/tools. "
                "Use mail_fetch_inbox to retrieve recent emails from inbox. "
                "Use mail_fetch_unanswered to retrieve unanswered emails only. "
                "Use mail_send when sending to an email address and no specific inbox message is referenced. "
                "Use mail_answer only when replying to an existing inbox message (mail_id/uid or explicit reference like 'reply to this email'). "
                "For mail_answer.mail_id use {steps[0].mail_id} or {steps[0].emails[0].uid}. "
                ...
                "If the user asks to read/analyze a PDF file, use read_pdf (not file_read). "
